...
const Collections = () => {
  ...
  const [ getCollections, { isLoading,error,data }, ] = useGetCollectionsMutation();
  const [ updateCategory, { isLoading: accessLoading, error: accessError, data: accessData }, ] = useUpdateCollectionMutation();
  ...
  const [togglor, setToggler] = useState({
   ...
   const updater = async (uniqeID, updateBool, action) => {
    await updateCategory({ uniqeID, updateBool, action }).unwrap();
  };
            ...
            <table className="w-full border-collapse rounded-full mt-5">
              ...
              <tr key={index} className="border-t">
                ...
                <td className="px-4 py-3">
                  <img src={item.pic} alt={item.name} className="w-10 h-10 rounded-full object-cover" />
                </td>
                       ...
                       <div
                        ...
                        >
                          ...
                          <td className="py-2 px-4 text-center items-center justify-center">
                            <i onClick={()=>navigate("/admin/Collection/manage", { state: { item }, })} className="ri-pencil-line text-[30px] opacity-45"></i>
                            &nbsp;&nbsp;&nbsp;
                            <i onClick={()=>updater(item._id, !togglor[item._id], "delete")} className="ri-delete-bin-line text-[30px] text-[#F0491B]"></i>
                          </td>
